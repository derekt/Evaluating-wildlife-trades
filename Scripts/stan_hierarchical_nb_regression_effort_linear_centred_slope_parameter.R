hierarchical_nb_regression_effort_linear_centred_slope_parameter <- 
"data {
...
model {
  ...
  for (taxa in 1:J)
  {
    ...
    segment(y, pos, timeseries_length[taxa]) ~ neg_binomial_2(exp(alpha[taxa] + slope[taxa] * segment(beta, pos, timeseries_length[taxa]) + effort_slope[taxa]*segment(gamma, pos, timeseries_length[taxa])), phi[taxa]);  //likelihood of the data distributed Poisson
...
for (taxa in 1:J)
{
  for (year in 1:timeseries_length[taxa]){
    ...
    // Predicted values used for calculating the linear regression credible intervals
      lambda[location] = exp(alpha[taxa] + beta[location] * slope[taxa] + effort_slope[taxa]*gamma[location]);
