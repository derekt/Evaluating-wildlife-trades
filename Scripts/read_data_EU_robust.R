...
legal <-
  read.csv(paste(data_directory, "eu_legal_sp_aggregations_taxa_term_agg-CAV_all-Terms.csv", sep = ""))
illegal <-
  read.csv(paste(data_directory, "eu_twix_sp_aggregations_taxa_term_agg-CAV_all-Terms.csv", sep = ""))

# Load the CITES Appendix information
CITES_appendix_eu = read.csv(paste(data_directory, "eu_legal_sp_aggregations_taxa_term_agg-CAV_CITES_Appendix.csv", sep=""))

years_to_model = 2005:2014
...
for (ii in 1:dim(legal)[1])
{
  ...
  CITES_appendix_eu[] <- lapply(CITES_appendix_eu, as.character)
  
  # Legal data extraction
  taxa_to_model = legal$Taxon[ii]
  class_to_model = legal$Class[ii]
  legal_term_to_model = legal$Term[ii]
  unit_to_fit = legal$Unit[ii]
  print(taxa_to_model)
  row_to_fit = ii
  full_legal_term = legal$Full.term.name[ii]
  
  # Add in the CITES data
  
  if (taxa_to_model == CITES_appendix_eu$Taxon[ii])
  {
    print(ii)
  } else {
  ...
  if (length(illegal_row_to_fit) > 0)
  {
    ...
    cols_to_fit = (which(colnames(illegal) == paste("X", min(years_to_model), sep =
                                                      ""))):(which(colnames(illegal) == paste("X", max(years_to_model), sep =
                                                                                                "")))
    # Extract the illegal columns to fit
    illegal_to_fit <-
      as.numeric(illegal[illegal_row_to_fit, cols_to_fit])
    ...
    CITES_appendix <- CITES_appendix_eu[row_to_fit, cols_to_fit]
